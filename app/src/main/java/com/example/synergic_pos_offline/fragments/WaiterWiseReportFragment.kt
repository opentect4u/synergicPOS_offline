...
/**
 * Waiter Wise Report - one waiter's bills over a period, a line each. Restaurant only
 * - see [ReportsFragment.isVisible], which gates the tile on Restaurant mode the same
 * way it gates KOT Cancel and the UDF reports.
 *
 ...
 * one person) asked of a different person.
 *
...
class WaiterWiseReportFragment : PeriodReportFragment<WaiterWiseReportDao.Report>() {
    ...
    override val filterOptions: List<String> by lazy { waiters.map { label(it) } }
    ...
    override fun load(fromDate: String, toDate: String): WaiterWiseReportDao.Report {
        ...
        // rather than quietly reporting on whichever waiter happened to be first.
            ?: return WaiterWiseReportDao.Report(fromDate, toDate, null, emptyList())
    ...
    override fun headline(report: WaiterWiseReportDao.Report): String =
        "${pretty(report.fromDate)}  to  ${pretty(report.toDate)}   •   " +
            "${report.waiter?.name.orEmpty()}   •   ${report.billCount} bill(s)"

    /**
     * The Shift Wise columns, operator among them (the cashier, not the waiter - the
     * waiter is the one already chosen in the picker above), and IGST/VAT only where
     * a bill in the period actually carried one.
     */
    ...
    /**
     * Tells a shop with no waiters on the master from a waiter that simply served
     * nothing, because only one of them is something to go and fix.
     */
    override fun emptyMessage(
        ...
        report.waiter == null -> "Pick a waiter" to
            "Choose which waiter to report on, then generate."
        else -> "No bills" to
    ...
    override fun printContent(report: WaiterWiseReportDao.Report): PeriodReportRenderer.Content =
        PeriodReportRenderer.Content(
            ...
            heading = listOf("WAITER" to (report.waiter?.let { label(it) } ?: "-")),
