...
object PrintLanguage {
    ...
    private fun translate(lang: Language, text: String): String {
        ...
        // A phrase written down whole wins over every heuristic below, even one that
        // would otherwise fire on it - "SCAN TO PAY" contains the joining word the
        // date-range case looks for, and without this check it came back "SCAN  से
        // PAY" instead of the sentence actually written down for it. This can only
        // ever short-circuit to the same answer [phrase] would give at the very end
        // of this function anyway, for a string one of the cases below would
        // otherwise have intercepted first.
        word(lang, body)?.let { return lead + it }
    ...
    private val WORDS: Map<String, Array<String>> = buildMap {
        ...
        w("GRAND TOTAL", "कुल योग", "देय रक्कम", "மொத்தக் கூடுதல்", "সর্বমোট", "స్థూల మొత్తం", "ಒಟ್ಟು ಮೊತ್ತ", "કુલ સરવાળો", "ସର୍ବମୋଟ", "ਕੁੱਲ ਜੋੜ", "সৰ্বমুঠ")
        // Overrides the plain TOTAL+TAX/TAX+AMOUNT combination for Hindi specifically -
        // a Hindi bill borrows "टैक्स" here rather than the dictionary word "कर" that
        // TAX alone still uses elsewhere (in "TOTAL SGST", "TAX%" and so on).
        w("TOTAL TAX", "कुल टैक्स", "एकूण कर", "மொத்த வரி", "মোট কর", "మొత్తం పన్ను", "ಒಟ್ಟು ತೆರಿಗೆ", "કુલ કર", "ମୋଟ କର", "ਕੁੱਲ ਟੈਕਸ", "মুঠ কৰ")
        w("TAX AMOUNT", "टैक्स रकम", "कर रक्कम", "வரித் தொகை", "করের টাকা", "పన్ను సొమ్ము", "ತೆರಿಗೆ ಮೊತ್ತ", "કર રકમ", "କର ରାଶି", "ਟੈਕਸ ਰਕਮ", "কৰৰ টকা")
        w("AMOUNT", "रकम", "रक्कम", "தொகை", "টাকা", "సొమ్ము", "ಮೊತ್ತ", "રકમ", "ରାଶି", "ਰਕਮ", "টকা")
        w("AMT", "रकम", "रक्कम", "தொகை", "টাকা", "సొమ్ము", "ಮೊತ್ತ", "રકમ", "ରାଶି", "ਰਕਮ", "টকা")
        w("QTY", "मात्रा", "प्रमाण", "அளவு", "পরিমাণ", "పరిమాణం", "ಪ್ರಮಾಣ", "જથ્થો", "ପରିମାଣ", "ਮਾਤਰਾ", "পৰিমাণ")
        w("QUANTITY", "मात्रा", "नग", "அளவு", "পরিমাণ", "పరిమాణం", "ಪ್ರಮಾಣ", "જથ્થો", "ପରିମାଣ", "ਮਾਤਰਾ", "পৰিমাণ")
        w("ITEM QTY", "आइटम की मात्रा", "नग", "பொருள் அளவு", "পণ্যের পরিমাণ", "వస్తువు పరిమాణం", "ವಸ್ತುವಿನ ಪ್ರಮಾಣ", "વસ્તુ જથ્થો", "ସାମଗ୍ରୀ ପରିମାଣ", "ਵਸਤੂ ਮਾਤਰਾ", "সামগ্ৰীৰ পৰিমাণ")
        w("PRICE", "कीमत", "किंमत", "விலை", "দাম", "ధర", "ಬೆಲೆ", "કિંમત", "ମୂଲ୍ୟ", "ਕੀਮਤ", "দাম")
        ...
        w("UNIT", "यूनिट", "यूनिट", "அலகு", "একক", "యూనిట్", "ಘಟಕ", "એકમ", "ଏକକ", "ਇਕਾਈ", "একক")
        w("PCS", "नग", "नग", "எண்ணிக்கை", "টুকরা", "నగలు", "ಸಂಖ್ಯೆ", "નંગ", "ନଗ", "ਨਗ", "টুকুৰা")
        w("KG", "किलो", "किलो", "கிலோ", "কেজি", "కిలో", "ಕೆಜಿ", "કિલો", "କିଲୋ", "ਕਿਲੋ", "কিলো")
        ...
        w("BILL NO", "बिल नंबर", "बिल क्र.", "பில் எண்", "বিল নং", "బిల్లు నం", "ಬಿಲ್ ಸಂ", "બિલ નં", "ବିଲ ନଂ", "ਬਿੱਲ ਨੰ", "বিল নং")
        ...
        w("S.NO", "क्र.सं.", "अ.क्र.", "வ.எண்", "ক্র.নং", "క్ర.సం", "ಕ್ರ.ಸಂ", "ક્રમ", "କ୍ର.ସଂ", "ਲੜੀ ਨੰ", "ক্ৰ.নং")
        w("SL NO", "अनुक्रमांक", "अनुक्रमांक", "வ.எண்", "ক্র.নং", "క్ర.సం", "ಕ್ರ.ಸಂ", "ક્રમ", "କ୍ର.ସଂ", "ਲੜੀ ਨੰ", "ক্ৰ.নং")
        ...
        w("MOBILE NO.", "मोबाइल नं", "मोबाईल नं", "கைபேசி எண்", "মোবাইল নং", "మొబైల్ నం", "ಮೊಬೈಲ್ ಸಂ", "મોબાઇલ નં", "ମୋବାଇଲ ନଂ", "ਮੋਬਾਈਲ ਨੰ", "মোবাইল নং")
        ...
        w("DISCOUNT", "डिस्काउंट", "डिस्काउंट", "தள்ளுபடி", "ছাড়", "తగ్గింపు", "ರಿಯಾಯಿತಿ", "ડિસ્કાઉન્ટ", "ରିହାତି", "ਛੋਟ", "ৰেহাই")
        w("DISC", "डिस्काउंट", "डिस्काउंट", "தள்ளுபடி", "ছাড়", "తగ్గింపు", "ರಿಯಾಯಿತಿ", "ડિસ્કાઉન્ટ", "ରିହାତି", "ਛੋਟ", "ৰেহাই")
        w("DISC.", "डिस्काउंट", "डिस्काउंट", "தள்ளுபடி", "ছাড়", "తగ్గింపు", "ರಿಯಾಯಿತಿ", "ડિસ્કાઉન્ટ", "ରିହାତି", "ਛੋਟ", "ৰেহাই")
        w("NET AMT", "शुद्ध राशि", "निव्वळ रक्कम", "நிகரத் தொகை", "নিট টাকা", "నికర సొమ్ము", "ನಿವ್ವಳ ಮೊತ್ತ", "ચોખ્ખી રકમ", "ନିଟ ରାଶି", "ਸ਼ੁੱਧ ਰਕਮ", "নিট টকা")
        w("ROUND OFF", "राउंड-ऑफ", "राउंड-ऑफ", "முழுமையாக்கல்", "রাউন্ড অফ", "రౌండ్ ఆఫ్", "ರೌಂಡ್ ಆಫ್", "રાઉન્ડ ઓફ", "ରାଉଣ୍ଡ ଅଫ", "ਰਾਊਂਡ ਆਫ", "ৰাউণ্ড অফ")
        w("ROUNDED OFF", "राउंड-ऑफ", "राउंड-ऑफ", "முழுமையாக்கல்", "রাউন্ড অফ", "రౌండ్ ఆఫ్", "ರೌಂಡ್ ಆಫ್", "રાઉન્ડ ઓફ", "ରାଉଣ୍ଡ ଅଫ", "ਰਾਊਂਡ ਆਫ", "ৰাউণ্ড অফ")
        ...
        w("PAY MODE", "पेमेंट", "पेमेंट", "கட்டண முறை", "পেমেন্ট", "చెల్లింపు", "ಪಾವತಿ", "ચુકવણી", "ଦେୟ", "ਭੁਗਤਾਨ", "পৰিশোধ")
        ...
        w("ONLINE", "ऑनलाइन", "ऑनलाइन", "ஆன்லைன்", "অনলাইন", "ఆన్‌లైన్", "ಆನ್‌ಲೈನ್", "ઓનલાઈન", "ଅନଲାଇନ", "ਆਨਲਾਈਨ", "অনলাইন")
        w("CARD", "कार्ड", "कार्ड", "கார்டு", "কার্ড", "కార్డ్", "ಕಾರ್ಡ್", "કાર્ડ", "କାର୍ଡ", "ਕਾਰਡ", "কাৰ্ড")
        w("SCAN TO PAY", "पेमेंट के लिए स्कैन करें", "पेमेंटसाठी स्कॅन करा", "பணம் செலுத்த ஸ்கேன் செய்யவும்", "পেমেন্টের জন্য স্ক্যান করুন", "చెల్లించడానికి స్కాన్ చేయండి", "ಪಾವತಿಸಲು ಸ್ಕ್ಯಾನ್ ಮಾಡಿ", "ચુકવણી માટે સ્કેન કરો", "ପେମେଣ୍ଟ ପାଇଁ ସ୍କାନ କରନ୍ତୁ", "ਭੁਗਤਾਨ ਲਈ ਸਕੈਨ ਕਰੋ", "পৰিশোধৰ বাবে স্কেন কৰক")
        ...
        // What the bill calls the person who rang it up. "CREATED BY" above is kept
        // because reports still use it - a report is generated, a sale is served.
        //
        // The borrowed word, not the dictionary one, in most of these: a shop says
        // कॅशियर, and खजांची reads like something out of a ledger from 1950 - the same
        // spelling in Hindi and Marathi both, per the shop's own printed bills. The
        // same choice REPORT makes just below, for the same reason. Tamil is the
        // exception - காசாளர் is the ordinary word there and is what a Tamil bill
        // would say.
        w("CASHIER", "कॅशियर", "कॅशियर", "காசாளர்", "ক্যাশিয়ার", "క్యాషియర్", "ಕ್ಯಾಷಿಯರ್", "કેશિયર", "କ୍ୟାଶିଅର", "ਕੈਸ਼ੀਅਰ", "কেশিয়াৰ")
        ...
        w("DUPLICATE", "डुप्लीकेट", "डुप्लिकेट", "நகல்", "নকল", "నకలు", "ನಕಲು", "ડુપ્લિકેટ", "ନକଲ", "ਡੁਪਲੀਕੇਟ", "নকল")
        ...
        // USER on its own is what makes "USER ID" combine to "यूज़र ID" (ID is a kept
        // acronym) - "USER NAME" is its own override just below, since a plain
        // combination is not the grammatical phrase either language actually uses.
        w("USER", "यूज़र", "यूज़र", "பயனர்", "ইউজার", "యూజర్", "ಬಳಕೆದಾರ", "યુઝર", "ୟୁଜର", "ਯੂਜ਼ਰ", "ইউজাৰ")
        w("USER NAME", "यूज़र का नाम", "वापरकर्त्याचे नाव", "பயனர் பெயர்", "ইউজারের নাম", "యూజర్ పేరు", "ಬಳಕೆದಾರರ ಹೆಸರು", "યુઝરનું નામ", "ୟୁଜରର ନାମ", "ਯੂਜ਼ਰ ਦਾ ਨਾਮ", "ইউজাৰৰ নাম")
        w("REPORT COUNT", "रिपोर्ट संख्या", "अहवाल संख्या", "அறிக்கை எண்ணிக்கை", "রিপোর্ট সংখ্যা", "నివేదికల సంఖ్య", "ವರದಿ ಸಂಖ್ಯೆ", "રિપોર્ટ સંખ્યા", "ରିପୋର୍ଟ ସଂଖ୍ୟା", "ਰਿਪੋਰਟ ਗਿਣਤੀ", "প্ৰতিবেদন সংখ্যা")
        // The full report title, not the shorter "UDF-WISE" tag used elsewhere (a
        // list entry, say) - the two read differently in each language and are kept
        // apart rather than one standing in for the other.
        w("UDF-WISE REPORT", "UDF के अनुसार रिपोर्ट", "UDF नुसार अहवाल", "UDF அடிப்படையிலான அறிக்கை", "UDF অনুযায়ী রিপোর্ট", "UDF ప్రకారం నివేదిక", "UDF ಪ್ರಕಾರ ವರದಿ", "UDF મુજબનો રિપોર્ટ", "UDF ଅନୁସାରେ ରିପୋର୍ଟ", "UDF ਮੁਤਾਬਕ ਰਿਪੋਰਟ", "UDF অনুসৰি প্ৰতিবেদন")
        ...
        w("TABLE", "टेबल", "टेबल", "மேசை", "টেবিল", "టేబుల్", "ಟೇಬಲ್", "ટેબલ", "ଟେବୁଲ", "ਟੇਬਲ", "টেবুল")
        // Overrides KOT + NO's own plain combination ("KOT नं"/"KOT क्र") - a KOT
        // ticket spells the number out.
        w("KOT NO", "KOT नंबर", "KOT क्रमांक", "KOT எண்", "KOT নং", "KOT నం", "KOT ಸಂ", "KOT નં", "KOT ନଂ", "KOT ਨੰ", "KOT নং")
        w("TAKE AWAY TOKEN", "टेक-अवे टोकन", "टेक-अवे टोकन", "டேக்அவே டோக்கன்", "টেক-অ্যাওয়ে টোকেন", "టేక్ అవే టోకెన్", "ಟೇಕ್ ಅವೇ ಟೋಕನ್", "ટેક-અવે ટોકન", "ଟେକ-ଅୱେ ଟୋକନ", "ਟੇਕ-ਅਵੇ ਟੋਕਨ", "টেক-এৱে টোকেন")
