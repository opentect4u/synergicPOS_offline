...
/**
 * Sends a generated Item Wise Report to the printer.
 *
 * Takes the report it is given rather than reading the period again: the operator
 * is printing what is on the screen in front of them.
 */
object ItemWiseReportPrinter {

    /**
     * How many item lines will be sent to a roll in one job.
     *
     * A line per item, so this is metres of paper already, and the whole report is
     * rendered to a single bitmap before any of it is sent - running out of memory
     * mid-job would be a worse answer than this one. Higher than the bill wise
     * report's limit because an item takes one line where a bill takes two.
     */
    const val MAX_PRINTABLE_ITEMS = 300

    /** @param result told what happened, so a caller can toast it wherever it is */
    fun print(context: Context, report: ItemWiseReportDao.Report, result: (String) -> Unit) {
        if (report.itemCount > MAX_PRINTABLE_ITEMS) {
            result(
                "${report.itemCount} items is too many to print at once - " +
                    "narrow the date range to $MAX_PRINTABLE_ITEMS or fewer"
            )
            return
        }
    ...
    private fun send(
        ...
        val capture = ItemWiseReportRenderer(context)
