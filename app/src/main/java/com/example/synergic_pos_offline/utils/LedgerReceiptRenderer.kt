...
/** Width the ledger card is laid out at for 80mm paper, matching receipt_ledger.xml. */
...
class LedgerReceiptRenderer(context: Context) {
    ...
    fun populate(
        ...
        try {
            ...
            rows.addView(divider())
            rows.addView(amountRow("TOTAL DUE", money(ledger.closing), bold = true, valueSize = 18f))
    ...
    private fun amountRow(label: String, value: String, bold: Boolean = false, valueSize: Float = 13f): View {
        val density = ctx.resources.displayMetrics.density
        return LinearLayout(ctx).apply {
            orientation = LinearLayout.HORIZONTAL
    ...
    private companion object {
        ...
        val SIZES = listOf(12f, 11f, 10f, 9f, 8f, 7f)
    }
}
